...
<x-online-layout :tenant="$tenant" title="Pilih Lokasi" active="menu">
    <div class="mx-auto max-w-5xl px-4 sm:px-6">
        <div class="mb-6">
            <h1 class="text-2xl font-extrabold leading-tight text-[#001356]">Pilih Lokasi Pengiriman</h1>
            <p class="mt-2 text-sm text-[#454650]">Pilih lokasi Anda untuk melihat menu dan harga yang tersedia di area tersebut.</p>
        </div>
        ...
        <div id="delivery-location-root">
            <div id="location-initial-state" class="rounded-2xl border border-dashed border-[#c6c5d2] bg-white p-6 text-center">
                ...
                <p class="mt-4 text-lg font-extrabold text-[#171c20]">Pilih Lokasi Pengiriman</p>
                <p class="mt-2 text-sm leading-5 text-[#454650]">Anda tetap bisa melihat menu meski GPS ditolak. Cari alamat atau pilih titik di peta.</p>
            ...
            <div id="location-selected-state" class="hidden">
                ...
                <div class="rounded-2xl border border-[#c6c5d2] bg-white p-4 shadow-sm">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <p class="text-sm font-bold text-[#171c20]">Lokasi Terpilih</p>
                    </div>
                    ...
                    <div class="mt-3 space-y-2">
                        <button id="use-current-location-button" type="button" class="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-[#c6c5d2] bg-white px-4 text-sm font-extrabold text-[#001356] transition active:scale-[0.98]">
                            <span class="material-symbols-outlined text-[20px]">my_location</span>
                ...
                <div id="coverage-success" class="mt-3 hidden rounded-xl border border-[#b9e3c8] bg-[#f0fdf4] px-4 py-3">
                    <div class="flex items-center gap-2">
                        ...
                        <p class="text-xs font-semibold leading-5 text-[#16a34a]">Lokasi berada dalam area pengiriman</p>
                    </div>
                </div>
                ...
                <button id="confirm-location-button" type="button" class="mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#001356] px-4 text-sm font-extrabold text-white shadow-sm transition active:scale-[0.98]">
                    <span data-confirm-label>Lihat Menu</span>
            ...
            <div class="flex h-full flex-col">
                ...
                <div class="relative min-h-0 flex-1">
                    <div class="absolute inset-x-4 top-4 z-[500]">
                        <div class="relative">
                            <input id="map-search-input" type="text" autocomplete="off" class="h-12 w-full rounded-2xl border border-[#c6c5d2] bg-white px-4 pr-12 text-base shadow-lg focus:border-[#001356] focus:ring-[#001356]" placeholder="Cari alamat, gedung, atau tempat...">
                            <button id="map-search-clear" type="button" class="absolute right-3 top-1/2 hidden -translate-y-1/2 rounded-full p-1 text-[#767681] transition hover:bg-[#dfe3e9] hover:text-[#001356]">
    ...
    @php
        $onlineCheckoutDeliveryConfig = [
            ...
            'catalogUrl' => route('online-orders.catalog', $tenant),
            'oldValues' => [
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            if (typeof initLocationSelector === 'function') {
                initLocationSelector(@json($tenant), @json($onlineCheckoutDeliveryConfig));
            }
        });
    </script>
</x-online-layout>
